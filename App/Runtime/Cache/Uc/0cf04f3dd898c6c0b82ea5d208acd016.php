...
<link rel="stylesheet" href="<?php echo STATIC_SERVER_URL;?>/usercenter/tp/css/my_help.css?0">
<div id='getAjaxPage' data-url="/Uc/Concur/ajaxGetConcur" data-uid="<?php echo ($uid); ?>" data-type="<?php echo ($type); ?>"><img src="http://static.gy.com/public/images/loading.gif"/></div>
...
<script type="text/javascript">$(function(){
    ...
    var type =$("#getAjaxPage").attr('data-type');
    $.ajax({
        url: url,
	...
	var tag;
	$(".hc_mrt_nav a").live('click',function(){
		$(this).addClass("nav_curr").siblings().removeClass("nav_curr");
		tag=$(this).attr("data-tag");
		$.ajax({
			url:url,
    		data:{uid:uid,type:type,tag:tag},
    		success:function(d){
    			 $("#getAjaxPage").html(d);
            	 $(".wraper_paging a:contains(1):first").addClass('selected');
    		}
